 ...
 */
add_action( 'init', 'mlm_default_settings_install', 90 );
function mlm_default_settings_install()
{
	if( is_admin() )
	{
		global $pagenow;
 ...
 * Admin Interface
 */
add_action( 'admin_menu', 'mlm_siteoptions_add_admin' , 999 );
function mlm_siteoptions_add_admin()
{
    global $query_string;

    if( isset($_REQUEST['page']) && $_REQUEST['page'] == 'mlm-settings' )
	{
		if( isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save'] )
		{
			$options = get_option('of_template');
			mlm_of_reset_options( $options, 'mlm-settings' );
			header("Location: admin.php?page=mlm-settings&reset=true");
			die;
		}
    }

	$tt_page	= add_submenu_page(
		'mlm-wallet',
		__( 'settings', 'mlm' ),
		__( 'settings', 'mlm' ),
		'edit_theme_options',
		'mlm-settings',
		'mlm_settings_page_callback'
	);
	/*
	$tt_page	= add_theme_page(
		__( 'settings', 'mlm' ),
		__( 'settings', 'mlm' ),
		'edit_theme_options',
		'mlm-settings',
	...
	*/
	...
	add_action( "admin_print_scripts-$tt_page", 'mlm_of_load_only' );
	add_action( "admin_print_styles-$tt_page", 'mlm_of_style_only' );
}
 ...
 * Reset Function
 */
function mlm_of_reset_options( $options,$page = '' )
{
	$template = get_option('of_template');
 ...
 */
function mlm_settings_page_callback()
{
	$options	= get_option('of_template');
	$return		= mlm_settings_machine( $options );
	?>
        <div class="wrap mlm-wrap mlm-settings-wrap clearfix" id="mlm_settings_wrapper">
            <h1 class="wp-heading-inline"><?php _e( 'Settings', 'mlm' ); ?></h1>
            <hr class="wp-header-end">
            <div id="of-popup-save" class="of-save-popup">
                <div class="of-save-save"><?php _e( 'Settings saved.', 'mlm' ); ?></div>
            </div>
            <div id="of-popup-reset" class="of-save-popup">
                <div class="of-save-reset"><?php _e( 'Reset settings', 'mlm' ); ?></div>
            </div>
            <form action="" enctype="multipart/form-data" id="ofform">
                <div id="main" class="clearfix">
                    <div id="of-nav" class="clearfix">
                ...
                <div class="save_bar_top">
                    <img style="display:none;float: left;" src="<?php echo IMAGES; ?>/spin.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
                    <input type="submit" value="<?php _e( 'Save settings', 'mlm' ); ?>" class="button-primary" />
            </form>
            <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
			<span class="submit-footer-reset">
				<input name="reset" type="submit" value="<?php _e( 'Reset settings', 'mlm' ); ?>" class="button submit-button reset-button" onclick="return confirm('<?php _e( 'Caution: all settings will reset. to confirm click on "Reset" button.', 'mlm' ); ?>');" />
 ...
 */
function mlm_of_style_only()
{
	wp_enqueue_style( 'options-style', STYLES .'/options.css' );
	wp_enqueue_style( 'color-picker', STYLES .'/colorpicker.css' );
}
 ...
 */
function mlm_of_load_only()
{
	add_action( 'admin_head', 'mlm_of_admin_head' );
	wp_enqueue_script( 'jquery-ui-core' );
	wp_register_script( 'jquery-input-mask', SCRIPTS .'/jquery.maskedinput-1.2.2.js', array( 'jquery' ) );
	wp_enqueue_script( 'jquery-input-mask' );
	wp_enqueue_script( 'color-picker', SCRIPTS .'/colorpicker.js', array('jquery') );
	wp_enqueue_script( 'ajaxupload', SCRIPTS .'/ajaxupload.js', array('jquery') );

	function mlm_of_admin_head()
	{
		?>
		...
		<script type="text/javascript">
			jQuery(document).ready(function(){
				...
				jQuery('#expand_options').click(function(){
					if(flip == 0){
						flip = 1;
						jQuery('#mlm_settings_wrapper #of-nav').hide();
						jQuery('#mlm_settings_wrapper #content').width(755);
						jQuery('#mlm_settings_wrapper .group').add('#mlm_settings_wrapper .group h2').show();
						jQuery(this).text('[-]');
					} else {
						flip = 0;
						jQuery('#mlm_settings_wrapper #of-nav').show();
						jQuery('#mlm_settings_wrapper #content').width(579);
						jQuery('#mlm_settings_wrapper .group').add('#mlm_settings_wrapper .group h2').hide();
						jQuery('#mlm_settings_wrapper .group:first').show();
						jQuery('#mlm_settings_wrapper #of-nav li').removeClass('current');
						jQuery('#mlm_settings_wrapper #of-nav li:first').addClass('current');
						jQuery(this).text('[+]');
					}
				...
				jQuery('.image_upload_button').each(function(){
					...
					new AjaxUpload(clickedID, {
						...
						onSubmit: function(file, extension){
							clickedObject.text("<?php _e( 'Loading', 'mlm' ); ?>"); // change button text, when user selects file
							this.disable(); // If you want to allow uploading only 1 file at time, you can disable upload button
							interval = window.setInterval(function(){
								var text = clickedObject.text();
								if (text.length < 13){	clickedObject.text(text + '.'); }
								else { clickedObject.text("<?php _e( 'Loading', 'mlm' ); ?>"); }
							}, 200);
						},
						onComplete: function(file, response) {
							window.clearInterval(interval);
							clickedObject.text("<?php _e( 'Upload image', 'mlm' ); ?>");
				...
				jQuery('#ofform').submit(function(){
					...
					var data = {
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'mlm-settings'){ ?>
						type: 'options',
						<?php } ?>
						action: 'of_ajax_post_action',
...
/* Ajax Save Action
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_of_ajax_post_action', 'mlm_of_ajax_callback');
function mlm_of_ajax_callback() {
					...
					if ( is_array($type)){
						foreach($type as $array){
							if($array['type'] == 'text'){
								...
								update_option( $id, esc_attr($new_value));
							}
						}
					}
...
/*-----------------------------------------------------------------------------------*/
function mlm_settings_machine( $options )
{
	$counter	= 0;
	$menu		= '';
	$output		= '';
		...
		switch( $value['type'] )
		{
			...
			case 'select':
				...
				foreach( $value['options'] as $option )
				{
					...
					$output .= '<option value="'. $option[0] .'"'. $selected .'>';
					$output .= $option[1];
					$output .= '</option>';
				}
			...
			case "upload":
				$output .= mlm_settings_uploader_callback($value['id'],$value['std'],null);
				break;

			case "upload_min":
				$output .= mlm_settings_uploader_callback($value['id'],$value['std'],'min');
				break;
...
/* File Uploading
/*-----------------------------------------------------------------------------------*/
function mlm_settings_uploader_callback( $id, $std, $mod )
{
	$uploader	= '';
	...
	$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">'. __( 'Upload image', 'mlm' ) .'</span>';
	...
	$uploader .= '<span class="button image_reset_button '. $hide.'" id="reset_'. $id .'" title="' . $id . '">'. __( 'Clear', 'mlm' ) .'</span>';
	$uploader .='</div>' . "\n";
