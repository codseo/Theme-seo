...
$mlm_title = $mlm_content = $mlm_thumb = $mlm_price = $mlm_size = $mlm_percent = $mlm_reject = $mlm_sale_price = $mlm_type = $mlm_count = $mlm_part = $mlm_author = $mlm_format = $mlm_language = $mlm_step = $mlm_button_text = $mlm_button_link = $mlm_button_2_text = $mlm_button_2_link =  '';
...
if( $post_id )
{
	...
	else
	{
		...
		$mlm_files			= get_post_meta( $post_id, '_downloadable_files', true );
...
if( ! is_array( $mlm_files ) )
{
	$mlm_files = array( 0 => array( 'name' => '', 'file' => '' ) );
}
...
<?php else: ?>
	...
	<div class="alert alert-danger text-justify">
		...
		<?php else: ?>
			...
			- <?php _e( 'You can not copy files from other sites', 'mlm' ); ?><br />
	...
	<form id="mlm_submit_product_form" action="<?php echo $products_url; ?>" method="post">
		<div class="form-group">
			...
			<div class="row no-gutters mx-n2">
				<div class="col-12 col-sm-6 px-2">
					<button type="button" class="btn btn-danger btn-block mt-2" data-toggle="modal" data-target="#mlm_product_uploads"><?php _e( 'Upload product file', 'mlm' ); ?></button>
				</div>
				<?php if( $demo == 'zhaket' ): ?>
					<div class="col-12 col-sm-6 px-2">
		...
		<div class="modal fade" id="mlm_product_uploads" tabindex="-1" role="dialog" aria-hidden="true">
			<div class="modal-dialog modal-lg" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title"><?php _e( 'Upload product files', 'mlm' ); ?></h5>
						<button type="button" class="close mr-auto ml-0" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
					<div class="modal-body mlm-upload-group">
						<?php
						$i		= 0;
						$ftp	= mlm_ftp_upload();
						$nonce	= wp_create_nonce('mlm_bngadsrwa');
						?>
						<?php foreach( (array) $mlm_files as $att ): ?>
							<div class="mlm-file-template mb-3">
								<div class="input-group mb-1">
									<input type="text" name="mlm_file[i][<?php echo $i; ?>][name]" class="name form-control" value="<?php echo $att['name']; ?>" placeholder="<?php _e( 'File name', 'mlm' ); ?>">
									<div class="input-group-append">
										<button class="mlm-remove-upload-btn btn btn-outline-danger" type="button"><?php _e( 'Delete', 'mlm' ); ?></button>
									</div>
								</div>
								<div class="input-group">
									<input type="text" name="mlm_file[i][<?php echo $i; ?>][file]" class="file form-control ltr" value="<?php echo $att['file']; ?>" placeholder="<?php _e( 'Upload or insert download link', 'mlm' ); ?>">
									<div class="input-group-append">
										<?php if( $ftp ): ?>
											<div class="mlm-ftp-upload-holder">
												<input type="file" class="upload-toggle" data-verify="<?php echo $nonce; ?>">
							...
							<?php $i++; ?>
						<?php endforeach; ?>
						<button type="button" class="mlm-new-upload-field btn btn-primary btn-block"><?php _e( 'Add new field', 'mlm' ); ?></button>
						<?php if( $ftp ): ?>
